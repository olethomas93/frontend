<template>
  <div style="width:100%;height:100%;" class="ma-auto">
    <!-- <atvise-visu-v3
      ...
      style="height:100%"
      :base="getBase(dashboard.widgets[0])"
      :args="dashboard.widgets[0].params"
      :query="dashboard.widgets[0].params"
      :settings="JSON.stringify({widget: dashboard.widgets[0].settings})"
    /> -->
    <grid-layout
      v-if="dashboard.widgets && showGrid"
      v-show="showGrid"
    ...
    >
      ...
      >
        ...
        >
          <div style="width:100%;height:calc(100% - 36px);overflow:none;">
            <atvise-visu-v3
              :base="item.base"
              :args="item.params"
              :query="item.params"
...
import { APIdashboard, APIservice } from '@/global/APIService.js'
...
export default {
  ...
  mounted () {
    ...
    this.loadDisplay(this.display)
    // this.loadDash('ObjectTypes.PROJECT.BaseObjectTypeJMH.LocationType.default')
  ...
  methods: {
    ...
    getTitle (item) {
      ...
      return obj ? obj.value : item.i
    },
    setTitle (value, item) {
      // console.log('set title', value)
      // item = value
    },
    async loadDisplay (display) {
      try {
        this.dashboard = {}
        // console.log('Loading dash..............', display)
        // const data = await this.apiDashboard.getWidgetV4(this, display)
        // const doc = this.parseHTML(data.html)
        const data = await this.apiService.read(display)
        const doc = this.parseHTML(data.value)
